...
export default function FeaturedPage() {
  ...
  useEffect(() => {
    const fetchFeaturedSeries = async () => {
      try {
        ...
        if (response.ok) {
          const data = (await response.json()).map((item: FeaturedItem) => ({
            ...item,
            isFeatured: !!item.isFeatured,
          }));

          console.log("FeaturedPage: API data received:", data);
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    console.log("FeaturedPage: Filtered and sorted items:", filtered);
    return filtered;
