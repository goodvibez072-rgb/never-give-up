...
import { existsSync, mkdirSync, copyFileSync, unlinkSync, readFileSync, writeFileSync, chmodSync, statSync, renameSync } from "fs";
import { dirname, join } from "path";
import { fileURLToPath } from "url";
import { randomBytes } from "crypto";

// Resolve the default sessions path relative to the compiled bundle (dist/index.js)
// so the path is correct regardless of the working directory used to launch the process.
const _bundleDir = dirname(fileURLToPath(import.meta.url));
const _defaultSessionsPath = join(_bundleDir, "../data/sessions.db");
// Single resolved value used everywhere in this module
const RESOLVED_SESSIONS_PATH = process.env.SESSIONS_PATH || _defaultSessionsPath;
...
const sessionsBackupDir = join(dirname(RESOLVED_SESSIONS_PATH), "backups");
  ...
  try {
    if (existsSync(dbFilePath)) {
      ...
      try {
        ...
        renameSync(dbFilePath, corruptedMovePath);
...
class SQLiteSessionStore extends session.Store {
  ...
  constructor() {
    ...
    // Use SESSIONS_PATH env var if set (required for VPS absolute paths)
    const sessionDbPath = RESOLVED_SESSIONS_PATH;
  ...
  private initializeSessionDatabase(sessionDbPath: string): Database.Database {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        // Wait a bit before retrying (synchronous sleep for ESM)
...
function getOrCreateSessionSecret(): string {
  // 1. Honour explicit SESSION_SECRET env var (standard VPS/Docker pattern).
  //    This is the most reliable approach since file writes may fail on some hosts.
  if (process.env.SESSION_SECRET && process.env.SESSION_SECRET.length >= 32) {
    console.log('[sessions] ✅ Using SESSION_SECRET from environment variable');
    return process.env.SESSION_SECRET;
  }

  const dataDir = dirname(RESOLVED_SESSIONS_PATH);
...
export function getOrCreateCsrfSecret(): string {
  // 1. Honour explicit CSRF_SECRET env var (recommended for VPS deployments).
  if (process.env.CSRF_SECRET && process.env.CSRF_SECRET.length >= 32) {
    console.log('[csrf] ✅ Using CSRF_SECRET from environment variable');
