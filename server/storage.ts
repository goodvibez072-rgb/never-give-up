...
import { existsSync, mkdirSync, copyFileSync, unlinkSync, writeFileSync, renameSync } from "fs";
import { dirname, join } from "path";
import { fileURLToPath } from "url";
import { randomBytes } from "crypto";

// Resolve default data paths relative to the compiled bundle (dist/index.js) rather
// than process.cwd(). This means the database is always found at <project>/data/
// regardless of the working directory used when launching Node.js on the VPS.
const _bundleDir = dirname(fileURLToPath(import.meta.url));
const _defaultDbPath = join(_bundleDir, "../data/database.db");
const _defaultSessionsPath = join(_bundleDir, "../data/sessions.db");

// Database paths - honours env vars; falls back to bundle-relative defaults
const dbPath = process.env.DATABASE_PATH || _defaultDbPath;
const sessionsDbPath = process.env.SESSIONS_PATH || _defaultSessionsPath;
