...
// IMPORTANT: When running behind Nginx + Cloudflare, do NOT redirect here.
// A 301 redirect on a POST request (login/signup) causes most browsers to
// replay it as a GET, stripping the request body AND the CSRF token — this
// is the root cause of "CSRF validation failed" errors on signup/login.
// Nginx should be configured to redirect HTTP→HTTPS itself.
// We only redirect when Node.js is accessed DIRECTLY (no proxy headers present).
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  // Detect reverse-proxy presence: Nginx/Cloudflare always add at least one of these.
  // If any forwarding header exists the proxy is handling HTTPS termination — skip redirect.
  const isBehindProxy = !!(
  ...
  // Only redirect direct (non-proxied) HTTP connections to HTTPS
  if (!req.secure && req.protocol !== 'https' && req.hostname !== '127.0.0.1' && req.hostname !== 'localhost') {
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      // React components use inline style={} props extensively; 'unsafe-inline' is
      // required for them to render correctly. CSS-based XSS is low-risk compared
      // to script injection, so this is an acceptable trade-off for a React SPA.
      styleSrc: [
        "'self'",
