...
const sessionsBackupDir = (() => {
  const sp = process.env.SESSIONS_PATH || "./data/sessions.db";
  return join(dirname(sp), "backups");
})();
...
class SQLiteSessionStore extends session.Store {
  ...
  constructor() {
    ...
    // Falls back to ./data/sessions.db for local development
...
function getOrCreateSessionSecret(): string {
  ...
  const dataDir = dirname(sessionsDbPath);
